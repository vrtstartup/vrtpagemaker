...
angular.module('immersiveAngularApp')
    .directive('feedback', function($mdDialog, $window) {
        return {
...
function FeedbackController($scope, $window, $mdDialog, FBMain) {
    $scope.addFeedback = function(name, feedback) {
        ...
        if (feedback) {
            FBMain.addFeedback(name, feedback, userAgent, screenWidth, screenHeight).then(function() {
                $mdDialog.cancel();
            });
        }
    };
